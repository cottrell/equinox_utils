...
class MLP(Module):
    ...
    width_size: Union[int, List] = field(static=True)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        # WARNING: depth is the number of hidden layers. There are depth + 1 layers total.

        super().__init__(**kwargs)
        keys = jrandom.split(key, depth + 1)
        layers = []
        width_size_ = [width_size] * depth if isinstance(width_size, int) else width_size
        assert len(width_size_) == depth
        if depth == 0:
            # NOTE: width_size is ignored if depth == 0
            layers.append(Linear(in_size, out_size, use_final_bias, key=keys[0]))
        else:
            layers.append(Linear(in_size, width_size_[0], use_bias, key=keys[0]))
            for i in range(depth - 1):
                layers.append(Linear(width_size_[i], width_size_[i + 1], use_bias, key=keys[i + 1]))
            layers.append(Linear(width_size_[-1], out_size, use_final_bias, key=keys[-1]))
